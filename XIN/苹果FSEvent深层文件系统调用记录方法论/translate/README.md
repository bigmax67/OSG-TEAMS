...
## FSEvents 介绍
在OS X 中，FSEvents 记录了磁盘上文件系统对象（比如文件或目录）所发生的变化。它被TimeMachine 和 Splotlight 等操作系统的多个组件使用。

OS X 10.5和10.6仅能捕获与目录相关的事件， 从OS X 10.7开始引入文件事件。 类似于Windows系统的NTFS日志（该日志记录一直文件系统活动，并将数据存储在UsnJrnl:$J中），FSEvents也一直记录文件系统变化，并将数据存储在FSEvent日志文件中。

通过解析FSEvent日志，文件系统曾发生过的事件，如文件、目录、符号链接和硬链接的创建、删除、重命名、修改、权限更改等都可以被审计，并用于法庭取证。
...
| 记录标志 | 名称 | 描述 | 可能的场景 |
|---------|-----|------|----------|
|0x00000001|对象是目录|(路径表示的)对象指向一个目录||
...
|0x01000000|创建|创建了一个文件。注意这个纪录标志只用于文件，不用于目录|- 将文件从一个挂载的卷复制到另一个挂载的卷<br>- 使用终端命令touch<br>- 使用文件复制和粘贴
|
|0x02000000|移除|(路径表示的)对象被移除。这个记录标志可被设置于文件或目录|- 从一个已挂载的卷中移动文件到另外一个卷<br>- 清空垃圾篓<br>- 使用终端命令rm|
|0x04000000|索引节点元数据修改|(路径表示的)对象索引节点元数据已修改|- 文件或目录被锁定或解锁|
|0x08000000|重命名|(路径表示的)对象被重命名。注意，重命名意味着对象被更改名称或它被移动到了别的目录下|- 重命名一个文件或目录<br>- 在同一个卷中移动文件或目录<br>- 使用终端命令 ren 重命名文件或目录<br>- 使用终端命令 mv 在同一个卷中移动文件或目录|
...
|0x80000000|目录创建|创建了一个目录|- 使用终端命令mkdir<br>- 使用目录的粘贴和复制|
...
下图显示了发送到垃圾篓的文件和目录。 请注意，在"mask"列中，所有事件都包括"Renamed"。 "Renamed"的说法有点用词不当。"Renamed"掩码以两种方式分配给事件：

1. 文件的名称已更改。
2. 该文件从一个目录移动到另一个目录（仅本地移动，不能跨卷移动）。

对于垃圾篓中的目录，当文件发送到垃圾篓时，他们会收到一个"Renamed"掩码，因为它们已从卷上某个其他位置移动到垃圾篓。
...
#### 合并多项变更
FSEvents记录标志可以指示文件或目录发生多次更改。由于FSEvents API记录和存储变化的工作原理的特性，确定每个单独变化发生的顺序以及发生这些更改的次数所需的粒度是无法满足的。
...
还有一个好消息是，FSEvents确实记录了一个文件/目录被更改的事实，我们知道发生了什么样的更改，只是不知道顺序和频率。
